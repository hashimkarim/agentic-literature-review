...
import { FileCode2, FilePlus2, Plus, Save, History, Download, Trash2, Undo2, Redo2, Search, X, RotateCcw, BookmarkPlus, RefreshCw, ChevronLeft, Check, AlertTriangle } from "lucide-react";
import { undo, redo } from "@codemirror/commands";
import { openSearchPanel } from "@codemirror/search";
import type { EditorView } from "codemirror";
import { strToU8, zipSync } from "fflate";
import type { Manuscript, ManuscriptDocument, ManuscriptFile, ManuscriptHistoryEntry, Project } from "@litagent/contracts";
import { api } from "./api";
...
export default function WritingWorkspace({ projectId, projects, onProjectChange }: { projectId: string; projects: Project[]; onProjectChange: (id: string) => void }) {
  ...
  return <section className="writing-workspace" aria-label="Writing workspace">
    ...
    {document ? <ManuscriptEditor key={document.id} document={document} /> : <div className="writing-empty"><FileCode2 size={32} /><h2>{loading || active && !error ? "Loading manuscript..." : "No manuscripts"}</h2>{!loading && !active && !error && <button type="button" className="writing-primary" onClick={() => setCreating(true)}><Plus size={16} />New manuscript</button>}</div>}
...
function ManuscriptEditor({ document }: { document: ManuscriptDocument }) {
  ...
  useEffect(() => { historyRequest.current++; setHistory(null); setHistorical(null); setRemote(null); setError(null); }, [active]);
  ...
  return <div className="writing-document">
    <div className="writing-toolbar">
      ...
      <Tool label="Undo" disabled={busy || !!compare || !file} onClick={() => { if (editor.current) undo(editor.current); }}><Undo2 size={16} /></Tool>
      <Tool label="Redo" disabled={busy || !!compare || !file} onClick={() => { if (editor.current) redo(editor.current); }}><Redo2 size={16} /></Tool>
      <Tool label="Find in file" disabled={!!compare || !file} onClick={() => { if (editor.current) openSearchPanel(editor.current); }}><Search size={16} /></Tool>
      ...
      <Tool label="File history" disabled={busy || !file} aria-pressed={history !== null} onClick={() => history ? (setHistory(null), setHistorical(null)) : void run(loadHistory)}><History size={16} /></Tool>
    ...
    <div className={`writing-body${history !== null ? " has-history" : ""}`}>
      ...
      <main className="writing-source">
        {compare && file ? <><div className="writing-diff-heading"><span>{historical?.entry.label ?? (historical ? new Date(historical.entry.savedAt).toLocaleString() : "Saved on disk")}</span><span>Current draft</span><Tool label="Close comparison" onClick={() => { setHistorical(null); setRemote(null); }}><X size={16} /></Tool></div><TextComparison before={compare.content} after={file.content} />
          ...
          </div></> : file ? <TexEditor filePath={active} content={file.content} disabled={busy} onChange={(content) => session.edit(active, content)} onView={(view) => { editor.current = view; }} /> : <div className="writing-empty"><h2>No source files</h2><button type="button" onClick={() => setDialog("file")}><FilePlus2 size={16} />New file</button></div>}
      </main>

...
try {
  ...
  for (const width of [1440, 1920, 860, 390]) {
    ...
    await assistant.getByLabel("Assistant connection 1", { exact: true }).selectOption(descriptor.id);
    await assistant.getByLabel("Assistant model 1", { exact: true }).selectOption("fixture-one");
    await assistant.getByRole("button", { name: "Add model", exact: true }).click();
    await assistant.getByLabel("Assistant connection 2", { exact: true }).selectOption(descriptor.id);
    await assistant.getByLabel("Assistant model 2", { exact: true }).selectOption("fixture-two");
    const before = calls;

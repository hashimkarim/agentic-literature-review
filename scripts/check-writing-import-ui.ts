...
try {
  ...
  for (const width of (process.env.LITAGENT_TEST_WIDTHS ?? "1440,1920,860,390").split(",").map(Number)) {
    ...
    await page.getByRole("button", { name: "Compile", exact: true }).click(); await page.getByText("Build succeeded", { exact: true }).waitFor({ timeout: 30_000 });
  ...
  if (process.env.LITAGENT_TEST_WRITING_FOLDER) {
    ...
    console.log(`PASS local source copy: ${complete.files.length} text files, ${complete.assets?.length} assets, ${complete.folders?.length} folders; original hashes unchanged. Compiler compatibility is not claimed.`);
    await page.close();
  }

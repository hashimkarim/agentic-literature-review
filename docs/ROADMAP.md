...
| Writing Mode | Partial | Global linked documents, multi-file TeX editor, autosave/recovery, per-file history/diffs/restore, source ZIP, selected-text multi-model alternatives, restricted offline Linux compilation, split PDF preview/download, diagnostics/cancellation/last-good PDF | Cross-platform installer validation, source/PDF navigation, source/citation attachment, outline/storyline, evidence-grounded new prose and live generation quality |

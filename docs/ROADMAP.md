...
| Writing Mode | Partial | Global linked documents, folder/Overleaf ZIP import, file tree/tabs/assets, multi-file TeX editor, autosave/recovery, per-file history/diffs/restore, complete source ZIP, selected-text alternatives, offline XeLaTeX/Biber/glossary/font builds, split PDF preview/download, diagnostics/cancellation/last-good PDF | Cross-platform installer validation, source/PDF navigation, source/citation attachment, outline/storyline, evidence-grounded new prose and live generation quality |

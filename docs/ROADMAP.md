...
| Writing Mode | Partial | Global documents with reusable project links, search/filter, legacy migration, multi-file TeX editor, autosave/recovery, per-file history/checkpoints/diffs/restore, source ZIP, selected-text multi-model alternatives, restricted offline Linux TeX build API with diagnostics/cancellation/last-good PDF | Compilation/preview UI, cross-platform installer validation, source/citation attachment, outline/storyline, evidence-grounded new prose and live generation quality |

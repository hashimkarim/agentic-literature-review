...
source-grounded new prose is available through a dedicated assistant. Existing note/synthesis features alone
...
| WRITE-03 | Citation finder and claim support: Working selected-source MVP | P1 | Selected-paper/internal-source citation tasks, exact ID/quote/revision validation, model support review, evidence inspection, bibliography insertion and internal provenance footnotes implemented. Automatic discovery, external search and structured result-cell navigation remain open. | C05, C32, C33, Core |
| WRITE-04 | Outlines, storylines and drafts: Working command MVP | P1 | Reviewable outlines, argument storylines and new prose at the cursor or selected range with multiple alternatives implemented. Reorderable argument/evidence blocks and dedicated reverse-outline UI remain open. | C03, C31, C37, Core |
| WRITE-05 | Bounded writing commands: Working command MVP | P1 | Draft/rewrite/expand/shorten/simplify/formalize/grammar/citations/review, explicit mixed-source context, separate support review and revision-safe acceptance implemented. Fine-grained inline patches and live quality evaluation remain open. | C23, C32, C34, Core |
...
| WRITE-12 | Searchable code and experiment context: Working snapshot MVP | P1 | File/folder attachment snapshots for code, CSV/JSON results (including W&B exports), notes and benchmarks; explicit selection, hashes/lines/origin URLs, secret exclusions and coverage preview implemented. Live connectors, structured run provenance and automatic retrieval remain open. | Core |
| WRITE-13 | Audience/complexity slider: Working controls MVP | P1 | Four audience levels, word budget, independent jargon/math detail, language and style constraints recorded with requests. Real-provider semantic-preservation evaluation remains open. | Core |
| WRITE-14 | Grammar, style and terminology review: Working command MVP | P1 | Grammar/tone/simplification commands and review-only reports, candidate comparison and accept/reject implemented. Inline grammar patches, glossary and reusable style profiles remain open. | Core |
| WRITE-15 | Revision-safe suggestions and provenance: Working batch MVP | P1 | Batches retain manuscript/context/source revisions, exact evidence quotes, models, review/failure state, rejection and recoverable acceptance/history. Changed or removed sources block insertion. Finer-grained patch review remains open. | Core |
...
| WRITE-19 | Multi-model draft candidates: Working drafting MVP | P1 | Explicit SDK models/counts, same-model variants, exact context preview, cursor drafts/selected edits, durable batches, model support review, comparison, rejection, cancellation and history-backed acceptance tested with synthetic installed SDK transport. Live quality validation remains open. | Core |

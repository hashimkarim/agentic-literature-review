...
function fixture(result: (input: ProviderRunStartInput) => Promise<ProviderRunResult> = async (input) => completed(`Alternative from ${input.model}`)) {
  ...
  const document = store.create({ name: "Synthetic manuscript", projectIds: [projectId] });
  const initial = document.files.find((file) => file.path === "main.tex")!;
  const main = store.writeFile(document.id, { path: initial.path, content: "Before. A measured result with uncertainty. After.", expectedRevision: initial.revision });
...
it("generates independent same-model and multi-model drafts without changing source", async () => {
  ...
  expect(f.store.read(f.id).files.find((file) => file.path === "main.tex")).toEqual(f.main);
...
it("deduplicates retries by request ID before and after restart; changed requests conflict", async () => {
  const f = fixture();
  const batch = f.service.start(f.id, f.input);
  expect(f.service.start(f.id, f.input).id).toBe(batch.id);
  await f.service.idle();
  const restarted = new WritingCandidateService(new ManuscriptStore(f.root), f.runtime, f.validate);
  expect(restarted.start(f.id, f.input).status).toBe("completed");
  expect(f.runtime.startRun).toHaveBeenCalledTimes(3);
  expect(() => restarted.start(f.id, { ...f.input, instruction: "Different" })).toThrow(/request ID/);
});
...
it("keeps successful candidates after partial failure and never publishes malformed output", async () => {
  ...
  const batch = f.service.start(f.id, f.input); await f.service.idle();
  const saved = f.service.get(f.id, batch.id);
...
it("accepts only an explicit candidate, preserves alternatives and records history", async () => {
  const f = fixture(); const batch = f.service.start(f.id, f.input); await f.service.idle();
  const chosen = batch.candidates[2]!;
  const accepted = f.service.accept(f.id, batch.id, { candidateId: chosen.id, expectedRevision: f.main.revision });
  expect(accepted.content).toBe(f.main.content.slice(0, 8) + "Alternative from second" + f.main.content.slice(42));
  expect(f.service.get(f.id, batch.id).candidates).toHaveLength(3);
  expect(f.store.history(f.id, f.main.path)[0]?.reason).toBe("candidate");
  expect(f.service.accept(f.id, batch.id, { candidateId: chosen.id, expectedRevision: f.main.revision })).toEqual(accepted);
  expect(() => f.service.accept(f.id, batch.id, { candidateId: batch.candidates[0]!.id, expectedRevision: f.main.revision })).toThrow(/already selected/);
  f.store.writeFile(f.id, { path: f.main.path, content: f.main.content, expectedRevision: accepted.revision });
  expect(() => f.service.accept(f.id, batch.id, { candidateId: chosen.id, expectedRevision: f.main.revision })).toThrow(/since changed/);
});

it("rejects candidates against concurrent edits and keeps both versions", async () => {
  const f = fixture(); const batch = f.service.start(f.id, f.input); await f.service.idle();
  f.store.writeFile(f.id, { path: f.main.path, content: "New manual edit", expectedRevision: f.main.revision });
  expect(() => f.service.accept(f.id, batch.id, { candidateId: batch.candidates[0]!.id, expectedRevision: f.main.revision })).toThrow(/changed after generation/);
  expect(f.store.read(f.id).files.find((file) => file.path === f.main.path)?.content).toBe("New manual edit");
  expect(f.service.get(f.id, batch.id).accepted).toBeNull();
});
...
it("recovers an interrupted acceptance without applying a replacement twice", async () => {
  ...
  expect(() => f.service.accept(f.id, batch.id, request)).toThrow(/Disk full/);
  const before = f.store.read(f.id).files.find((file) => file.path === f.main.path)!;
  expect(f.service.accept(f.id, batch.id, request)).toEqual(before);
  expect(f.service.get(f.id, batch.id).accepted?.candidateId).toBe(request.candidateId);
  expect(f.store.history(f.id, f.main.path).filter((entry) => entry.reason === "candidate")).toHaveLength(1);
});
...
it("cancels active and queued work, ignores late output and does not start more calls", async () => {
  ...
  f.service.cancel(f.id, batch.id);
  ...
  expect(f.service.get(f.id, batch.id).candidates.every((candidate) => candidate.status === "cancelled" && candidate.text === null)).toBe(true);
});

it("reconciles a restart as interrupted, without replaying started or queued model calls", async () => {
  const f = fixture(); const batch = f.service.start(f.id, f.input); await f.service.idle();
  const stored = f.service.get(f.id, batch.id);
  ...
  expect(restarted.list(f.id)[0]?.status).toBe("interrupted");
  expect(restarted.get(f.id, batch.id).candidates.map((candidate) => candidate.status)).toEqual(["completed", "interrupted", "interrupted"]);
  expect(f.runtime.startRun).toHaveBeenCalledTimes(3);
});
...
it("enforces bounds, scope and explicit targets before starting any model work", () => {
  ...
  for (const input of [{ ...f.input, expectedRevision: "0".repeat(64) }, { ...f.input, to: 8001 }, { ...f.input, targets: [{ providerId: "codex", model: "first", count: 1 }] }]) {
    expect(() => f.service.start(f.id, input)).toThrow();
  }
  f.validate.mockImplementation(() => { throw new Error("Connection disabled"); });
  expect(() => f.service.start(f.id, f.input)).toThrow(/disabled/);
  expect(f.runtime.startRun).not.toHaveBeenCalled();
  expect(() => f.service.list("manuscript_0000000000000000")).toThrow(/not found/);
});

it("retains legacy candidate provenance and acceptance when documents move and projects are unlinked", async () => {
  const f = fixture();
  const batch = f.service.start(f.id, f.input); await f.service.idle();
  f.store.saveCandidateBatch({ ...f.service.get(f.id, batch.id), projectId: f.projectId });
  const { projectIds: _links, files: _files, ...metadata } = f.store.read(f.id);
  const legacy = path.join(f.root, "projects", f.projectId, "manuscripts", f.id);
  fs.mkdirSync(path.dirname(legacy), { recursive: true });
  fs.renameSync(path.join(f.root, "manuscripts", f.id), legacy);
  fs.writeFileSync(path.join(legacy, "manuscript.json"), JSON.stringify({ ...metadata, projectId: f.projectId }));
  const store = new ManuscriptStore(f.root);
  const service = new WritingCandidateService(store, f.runtime, f.validate);
  expect(store.list()).toMatchObject([{ id: f.id, projectIds: [f.projectId] }]);
  store.updateProjects(f.id, { projectIds: [], expectedProjectIds: [f.projectId] });
  expect(service.get(f.id, batch.id)).toMatchObject({ projectId: f.projectId, status: "completed" });
  const accepted = service.accept(f.id, batch.id, { candidateId: batch.candidates[0]!.id, expectedRevision: f.main.revision });
  expect(accepted.content).toContain("Alternative from first");
  expect(service.list(f.id)).toHaveLength(1);
  expect(f.runtime.startRun).toHaveBeenCalledTimes(3);
});
